...
context("test-stabJGL.R")

test_that("stabJGL returns objects of correct classes", {
...
test_that("stabJGL throws error", {
  ...
  # Test errors
  expect_error(stabJGL(list(dat$data, dat$data[1:n,1:(p-1)]),verbose=F)) # Different dimensions of data matrices
  expect_error(stabJGL(dat.list, ebic.gamma = -1, verbose = F)) # Negative ebic.gamma
  expect_error(stabJGL(dat.list, subsample.ratio = 1.2, verbose = F)) # Too large subsample ratio
  expect_error(stabJGL(dat.list, nCores = -1, verbose = F)) # Negative nCores
  expect_error(stabJGL(dat.list, nlambda1=1, verbose = F)) # Too few lambda1 values
  expect_error(stabJGL(dat.list, nlambda2=1, verbose = F)) # Too few lambda2 values
  expect_error(stabJGL(dat.list, lambda1.min=0.2, lambda1.max = 0.1, verbose = F)) # lambda1 max smaller than min
  expect_error(stabJGL(dat.list, lambda2.min=0.2, lambda2.max = 0.1, verbose = F)) # lambda2 max smaller than min
  expect_error(stabJGL(dat.list, var.thresh = 1.2 , verbose = F)) # Too large var.thresh
  expect_error(stabJGL(dat.list, rho=0 , verbose = F)) # Non-positive step size
  expect_error(stabJGL(dat.list, rep.num=0 , verbose = F)) # Non-positive number of subsamplings
  expect_error(stabJGL(dat.list, weights = 'blabla',verbose = F)) # Not valid weights argument
})

test_that("stabJGL default arguments", {
  # Example: The two data sets are identical
  set.seed(123)
  n <- 30
  p <- 5
...
test_that("stabJGL works given optional parameters", {
  # Example: The two data sets are identical
  set.seed(123)
  n <- 30
  p <- 5
  dat <- huge::huge.generator(n = n, d = p, graph = "scale-free", verbose = F)
  dat.list = list(dat$data, dat$data)
  expect_equal(class(stabJGL(dat.list, scale=FALSE,penalize.diagonal = TRUE, subsample.ratio=0.7,verbose = F)$opt.lambda1),'numeric')
  # One case with more observations
  n=150
  dat <- huge::huge.generator(n = n, d = p, graph = "scale-free", verbose = F)
  dat.list = list(dat$data, dat$data)
  expect_equal(class(stabJGL(dat.list, scale=FALSE,penalize.diagonal = TRUE,verbose = F)$opt.lambda1),'numeric')
})

test_that("stabJGL arguments control what they should", {
  # Example: The two data sets are identical
  set.seed(123)
  n <- 30
  p <- 5
  ...
  expect_true(mean(res$opt.sparsities) >= mean(stabJGL(dat.list, ebic.gamma = 1, verbose = F)$opt.sparsities)) # Test argument ebic.gamma controls sparsity
  expect_true(mean(res$opt.sparsities) >= mean(stabJGL(dat.list, var.thresh = 0.01, verbose = F)$opt.sparsities)) # Test argument var.thresh controls sparsity
  expect_output(stabJGL(dat.list, verbose = T)) # Test that verbose=T gives output
  expect_true(res$opt.ebic!= stabJGL(dat.list, verbose = FALSE, scale = FALSE)$opt.ebic) # Test that not scaling gives different results.
  expect_true(res$opt.ebic== stabJGL(dat.list, verbose = TRUE, parallelize = FALSE)$opt.ebic) # Test that parallelizing does not affect final estimate
  expect_true(any(res$total.variability != stabJGL(dat.list, retune.lambda1 = TRUE ,verbose = FALSE)$total.variability)) # Test that retuning changes the resulting variability of lambda1
})


test_that("stabJGL results are valid", {
  # Example: The two data sets are identical
  set.seed(123)
  n <- 30
  p <- 5
...
test_that("stabJGL results have correct lengths", {
  # Example: The two data sets are identical
  set.seed(123)
  n <- 30
  p <- 5
